...
export class posicion_guardada extends Phaser.Scene {
    constructor() {
        super({ key: 'ver_posicion' });
    }
    ...
    create () {
        ...
        this.add.text(774, 180, 'Posiciones Guardadas', {
            fontSize: '40px',
            fontStyle: 'bold',
            color: '#000000',
        }).setOrigin(0.5);
        ...
        fetch(`${apiUrl}/api/posicions`)
            .then(response => response.json())
            .then(data => {
                // Crear un contenedor en Phaser para la tabla
                const tableContainer = this.add.dom(500, 225).createElement('div');

                // Crear una tabla
                const table = document.createElement('table');

                // Crear encabezados de la tabla para ID, nombre, contraseña y tipo
                const headers = ['Evento', 'QTE', 'Jugador', 'Posicion X', 'Posicion Y', ' ', ' '];
                ...
                data.data.forEach(item => {
                    ...
                    const rowData = [
                        ...
                        attributes.QTE,
                        attributes.jugador,
                        attributes.posicionX,
                        attributes.posicionY,
                    ];
                    ...
                    img1.onclick = () => {
                        ...
                        localStorage.setItem('itemModificarPosicion', dataString)
                        this.scene.start('modificarPosicionScene')
                    };
                    ...
                    img2.onclick = () => {
                        ...
                        fetch(`${apiUrl}/api/posicions/${rowId}`, {
                            method: 'DELETE',
                            headers: {
                ...
                tableContainer.node.style.maxWidth = '1400px'; // Por ejemplo, un ancho máximo de 1450px
                tableContainer.node.style.height = '400px'; // Altura fija para la tabla
                ...
                Array.from(tableCells).forEach(cell => {
                    ...
                    cell.style.maxWidth = '500px'; // Establecer un ancho máximo para las celdas, por ejemplo, 200px
        ...
        const volverButton = this.add.dom(774, 680).createFromHTML(`<button style="${styleButtom}">Volver</button>`);
...
export default posicion_guardada;