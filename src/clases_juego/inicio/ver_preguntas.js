...
export class ver_Preguntas extends Phaser.Scene {
    constructor() {
        super({ key: 'ver_preguntas' });
    }
    ...
    create () {
        ...
        this.add.text(774, 100, 'Preguntas', {
            fontSize: '40px',
            fontStyle: 'bold',
            color: '#000000',
        }).setOrigin(0.5);
        ...
        fetch(`${apiUrl}/api/vendedor-preguntas`)
            ...
            .then(data => {
                ...
                const tableContainer = this.add.dom(60, 125).createElement('div');

                // Crear una tabla
                const table = document.createElement('table');

                // Crear encabezados de la tabla
                const headers = ['Contexto', 'Pregunta', 'Qué', 'Quién', 'Operaciones', 'Resultado', ' ', ' '];
                ...
                data.data.forEach(item => {
                    ...
                    img1.onclick = () => {
                        ...
                        console.log('Hiciste clic en la primera imagen de la fila con ID:', item.id);
                        const dataString = JSON.stringify(item);
                        const it = localStorage.setItem('itemModificar', dataString)
                        console.log(it)
                    ...
                    img2.onclick = () => {
                        ...
                        fetch(`${apiUrl}/api/vendedor-preguntas/${rowId}`, {
                            method: 'DELETE',
                            headers: {
                ...
                tableContainer.node.style.maxWidth = '1450px'; // Por ejemplo, un ancho máximo de 1450px
