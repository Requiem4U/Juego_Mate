...
function toRelative(path, dir) {
  let folders = split(vite.normalizePath(path), "/");
  folders = folders.slice(
    folders.findIndex((f) => f === dir),
    folders.length
  );
  return vite.normalizePath(`./${folders.join("/")}`);
}
function toCamelCase(string) {
  return string.trim().replace(/[-_\s]+(.)?/g, (_, c) => c ? c.toUpperCase() : "");
}
const logger = consola__default.create({ defaults: { message: "[vite-plugin-alias] -" } });
function abort(message) {
  throw logger.error(new Error(message));
}
async function readJSON(path) {
  try {
    const file = (await fs__default.readFile(path, "utf-8")).toString();
...
async function writeJSON(path, data, process) {
  ...
  try {
    await fs__default.writeFile(path, commentJson.stringify(data, null, 4));
...
async function writeConfig(gen, process = "default") {
  ...
  const file = vite.normalizePath(`${root}/${name}.json`);
  try {
...
async function getDirectories(gen) {
  ...
  const directories = await fg__default.sync(deep ? `${dir}/**/*` : `${dir}/*`, {
    ignore: ["node_modules"],
...
async function writeLog(gen, process = "default") {
  ...
  if (!createLog)
    return;
  const folder = vite.normalizePath(logPath);
  const file = vite.normalizePath(`${folder}/alias.json`);
...
class Generator {
  constructor(servermode, options) {
    ...
    this.fullPath = vite.normalizePath(node_path.resolve(this.options.root, this.options.dir));
    if (servermode === "serve")
      this.observe();
  }
  /**
  ...
  addAlias(path) {
    toArray(path).forEach((p) => {
      const correctedPath = vite.normalizePath(p);
      ...
      if (this.alias.some((a) => a.find === key)) {
        ...
        if (this.options.adjustDuplicates && this.options.depth > 1) {
          const name = folders.filter((f) => !split(vite.normalizePath(this.options.dir), "/").includes(f)).join("-");
   ...
   */
  observe() {
    const watcher = chokidar__default.watch(this.fullPath, { ignoreInitial: true, depth: this.options.depth });
